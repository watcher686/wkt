...
Page({
  ...
  onLoad: function (options) {
    that = this;
    that.getData(that.data.page, options);
  },
  /**
   * 获取列表数据
   * 
   */
  getData: function (page, options) {
    // 获取总数
    ...
    // 获取前十条
    try {
      console.log("传过来的openid：", options.openid)
      db.collection('history')
        .where({
          _openid: options.openid, // 填入当前用户 openid      that.data.openid
        })
